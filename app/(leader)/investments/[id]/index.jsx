...
export default function LeaderInvestmentDetails() {
  const router = useRouter();
  const { id } = useLocalSearchParams();
  const { axiosAuth: auth, user } = useAuth();   // ⭐ FIXED
  ...
  /* ---------------- FETCH LEADER'S OWN INVESTMENT ---------------- */
  const fetchInvestment = async () => {
    setLoading(true);
    try {
      const res = await auth().get(`/investments/${id}`);  // ⭐ Correct API
      setInvestment(res.data);
    } catch (err) {
      console.log("❌ Leader Investment Fetch Error:", err);
      router.back();
    } finally {
      setLoading(false);
    }
  ...
  /* -------- AUTORUN WHEN LEADER OR ID IS AVAILABLE -------- */
  useEffect(() => {
    if (!id || !user) return;
    fetchInvestment();
  }, [id, user]);
